...
# the ndu user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the ndu user to the desired the user (NB_USER)
    if id ndu &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" ndu 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the ndu user:"
            _log "- username: ndu       -> ${NB_USER}"
            _log "- home dir: /home/ndu -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the ndu user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the ndu home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "ndu" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/ndu to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/ndu/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/ndu to /home/${NB_USER}!"
                _log "Attempting to symlink /home/ndu to /home/${NB_USER}..."
                if ln -s /home/ndu "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/ndu to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/ndu/"* ]]; then
...
# user we started as.
    ...
    ndu_UID="$(id -u ndu 2>/dev/null)"  # The default UID for the ndu user
    ndu_GID="$(id -g ndu 2>/dev/null)"  # The default GID for the ndu user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old ndu user to nayvoj ($(id -u ndu):$(id -g ndu))"
            ...
            sed --expression="s/^ndu:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/ndu:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "ndu" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/ndu. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/ndu ]]; then
        _log "WARNING: no write access to /home/ndu. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
